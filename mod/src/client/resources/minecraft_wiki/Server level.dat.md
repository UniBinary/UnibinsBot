...
*页面ID: 23784* | *来源: Minecraft Wiki*

* *server_level.dat**是Minecraft Classic[服务器](服务器.md)用于加载和保存游戏内地图的文件的名称。可以备份该文件以保存内容，这有助于保护构造免受[攻击者](恶意破坏.md)或使用该文件进行地图编辑。

# 一般信息
使用[gzip](wzh:Gzip.md)压缩文件以节省空间（因为文件最终会因为一切而变得相当大，甚至[空气](空气.md)会被视为[方块](方块.md)）。

* *（注意：在此示例中，使用了默认的server_level.dat，大小为256×256×64。维度较大的地图文件的字节数可能不同。）**<br/>
server_level.dat文件是Minecraft[服务器](服务器.md)转储地图信息以进行永久存储的位置。由于此文件主要是原始地图数据，因此可能非常大：常规256×256×64大小的世界大小为4MB。但是，使用gzip压缩文件。大多数方块值为0（空白空间），可以通过压缩大大减小了大小，通常减少到几百KB。
...
数组的前65536个连续字节构成该地图的最顶部256×256“薄片”，最左边的字节行从左到右，位于0 ... 255的位置，下面的行从左到右，位于256 ... 511的位置，依此类推。默认地图深64“薄片”。
...
[en:server_level.dat](en:server_level.dat.md)
[fr:server_level.dat](fr:server_level.dat.md)