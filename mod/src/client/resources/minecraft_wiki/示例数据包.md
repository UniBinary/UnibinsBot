...
*页面ID: 121944* | *来源: Minecraft Wiki*

示例数据包是由[Slicedlime](Slicedlime.md)创建的用于展示游戏更新的[数据包](数据包.md)，目前共有两个数据包：
* [第16版（23w31a）数据包演示数据包](https://github.com/slicedlime/examples/tree/master/datapacks/v16demo)
* [数据驱动魔咒数据包](https://github.com/slicedlime/examples/tree/master/datapacks/enchantments)

# 第16版数据包演示数据包
此数据包共含有8个函数，用于展示在[23w31a](23w31a.md)里添加的新内容。
...
# 数据驱动魔咒数据包
此数据包共含有14个[魔咒](魔咒.md)，用于展示在[24w18a](24w18a.md)里添加的魔咒数据驱动支持。

# = Curse of Annoyance =

* *Curse of Annoyance**是一种使持有者在击中方块时有概率收到“猫的事实（Cat Facts）”的诅咒型魔咒。
...
* *Boom Boom**是一种使射出的箭在击中方块时产生半径为2的爆炸的魔咒。
...
* *Cowbow**是一种使弩的装填音效变为[牛](牛.md)的音效的魔咒。

附有该魔咒时，弩的装填时音效变为牛的空闲音效；装填完毕的音效变为牛的受伤音效。

# = Diminishing =

* *Diminishing**是一种使穿戴者变小的魔咒。

每级魔咒增加20%的缩小比例。

# = Fire Walker =

* *Fire Walker**是一种在[熔岩](熔岩.md)上行走可以产生[黑曜石](黑曜石.md)的魔咒。

在地上行走（不处于掉落、跳跃或飞行状态）时，以当前方块位置为圆心，等级+2为半径的区域内的所有暴露在空气下，且与玩家站着的方块高度相同的熔岩源方块都会变为黑曜石，其中有10%的概率变为[哭泣的黑曜石](哭泣的黑曜石.md)。等级达到14时覆盖范围达到最大，更高的等级将不会提升其覆盖范围。如果将要替换的熔岩源方块中有生物、船或玩家等实体，则该方块不会变成黑曜石。

此魔咒能消除[细雪](细雪.md)造成的伤害。

# = Fishy =

* *Fishy**是一种手持时有概率生成[蠹虫](蠹虫.md)的魔咒。

手持附有该魔咒的物品时，每刻以0.1%的概率尝试在使用-{}-者的位置生成1只蠹虫。

# = Curse of Fragility =

* *Curse of Fragility**是一种有概率额外消耗物品耐久度的诅咒型魔咒，可以缩短物品的使用寿命。

每次击中方块、进行攻击或受到攻击时有3%的概率触发效果。触发效果后，若为击中方块和受到攻击则损失64点耐久度；若为进行攻击则损失256点耐久度。

# = Galaxy Brain =

* *Galaxy Brain**是一种在击杀生物后额外获得100点[经验](经验.md)的魔咒。

魔咒等级不影响实际效果。

# = Multi-Multishot =

* *Multi-Multishot**是一种能够一次射出大量[箭](箭.md)的魔咒。

其效果与多重射击类似，但每级魔咒会额外装填20只箭。

# = Curse of Pollen Allergy =

* *Curse of Pollen Allergy**是一种在森林类生物群系时有概率伤害自己的诅咒型魔咒。

穿戴附有该魔咒的物品时，若穿戴者位于森林类生物群系，每刻以0.5%的概率尝试对穿戴者造成点荆棘伤害。

魔咒等级不影响实际效果。

# = Roulette =

* *Roulette**是一种以小概率损毁工具为代价，使工具不消耗耐久的魔咒。
...
* *Thor**是一种使持有者在[雷暴](雷暴.md)天气对伤害免疫的魔咒。
