...
--chunk, -c X Z         : 直接指定区块坐标 (X Z)
--player-coords, -p X Y Z : 玩家坐标，自动计算对应区块
--chunks X1 Z1 X2 Z2 ... : 批量读取多个区块
--list-regions, -l      : 列出存档中的所有区域文件
--verbose, -v           : 显示详细信息

=== 使用示例 ===
# 读取玩家坐标对应的区块
python mcapy_reader.py --save "存档路径" --player-coords 317 79 24

# 直接指定区块坐标
python mcapy_reader.py --save "存档路径" --chunk 19 1

# 批量读取区块
python mcapy_reader.py --save "存档路径" --chunks 19 1 20 1 19 2

# 列出区域文件
python mcapy_reader.py --save "存档路径" --list-regions

# 详细模式
python mcapy_reader.py --save "存档路径" --chunk 19 1 --verbose

=== 坐标计算 ===
玩家坐标 (317, 79, 24) → 区块坐标 (19, 1)
区块X = floor(317 / 16) = 19
区块Z = floor(24 / 16) = 1
"""
